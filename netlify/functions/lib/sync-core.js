// Lógica compartida: revisa el feed RSS público del Substack de Fraccctal y
// replica cada carta nueva como una fila en la tabla `cartas` de Supabase.
// La usan tanto sync-substack.js (automática, semanal) como
// sync-substack-manual.js (disparada a mano, protegida con secreto).
...
function parseFeed(xml) {
  ...
  return items
    .map((block) => {
      ...
      // Sacamos el botón de suscripción propio de Substack: ponemos el
      // nuestro, con nuestro estilo, al pie de carta.html.
...
async function runSync({ SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY }) {
  const feedRes = await fetch(FEED_URL);
  if (!feedRes.ok) {
    throw new Error("No se pudo leer el feed de Substack");
  }
  ...
  // De la más vieja a la más nueva, para que created_at quede en orden.
  ...
  return { revisadas: items.length, nuevas: nuevas.length, slugs: nuevas.map((i) => i.slug) };
}
